...
class PersonImagesAdapter (context: Context, var profileList: List<Profile>) : RecyclerView.Adapter<PersonImagesAdapter.ViewHolder>() {
    //this method is returning the view for each item in the list
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PersonImagesAdapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.row_person_images ,parent, false)
        return ViewHolder(v)
    }

    //this method is binding the data on the list
    override fun onBindViewHolder(holder: PersonImagesAdapter.ViewHolder, position: Int) {
